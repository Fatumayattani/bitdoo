...
export function Dashboard({ onBack }) {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-cyan-400 via-teal-400 to-blue-500 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        ].map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`py-4 px-4 border-b-4 font-bold text-lg transition-all duration-300 rounded-t-2xl ${activeTab === tab.id
          ...
          <div className="p-6">
            {activeTab === 'overview' && (<div className="space-y-6">
                ...
                <div className="space-y-4">
                  {myLoans.map((loan) => (<div key={loan.id} className="flex items-center justify-between p-6 bg-white bg-opacity-20 rounded-2xl backdrop-blur-sm shadow-lg border-2 border-white border-opacity-30 transform hover:scale-105 transition-all duration-300">
                      <div className="flex items-center space-x-4">
                        <div className={`w-3 h-3 rounded-full ${loan.status === 'active' ? 'bg-green-500' : 'bg-blue-500'}`}></div>
                        <div>
                      ...
                      <div className="text-right">
                        <p className={`text-lg font-bold ${loan.status === 'active' ? 'text-green-300' : 'text-blue-300'}`}>
            ...
            {activeTab === 'borrowing' && (<div className="space-y-6">
                <h3 className="text-2xl font-black text-white">🏦 Your Borrowed Loans</h3>
                <div className="space-y-4">
                  {myLoans.filter(loan => loan.type === 'borrowed').map((loan) => (<div key={loan.id} className="bg-white bg-opacity-20 rounded-3xl p-8 backdrop-blur-sm shadow-2xl border-2 border-white border-opacity-30 transform hover:scale-105 transition-all duration-300">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className={`px-4 py-2 rounded-2xl text-lg font-black shadow-lg ${loan.status === 'active' ? 'bg-green-400 text-white' : 'bg-blue-400 text-white'}`}>
                          {loan.status === 'active' ? 'Active' : 'Completed'}
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="w-full bg-white bg-opacity-30 rounded-full h-3 shadow-inner">
                          <div className="bg-gradient-to-r from-green-400 to-blue-500 h-3 rounded-full shadow-lg" style={{ width: `${loan.progress}%` }}></div>
                        </div>
                      </div>

                      {loan.nextPayment && (<div className="flex items-center justify-between">
            ...
            {activeTab === 'lending' && (<div className="space-y-6">
                <h3 className="text-2xl font-black text-white">💰 Your Funded Loans</h3>
                <div className="space-y-4">
                  {myLoans.filter(loan => loan.type === 'lent').map((loan) => (<div key={loan.id} className="bg-white bg-opacity-20 rounded-3xl p-8 backdrop-blur-sm shadow-2xl border-2 border-white border-opacity-30 transform hover:scale-105 transition-all duration-300">
                      <div className="flex justify-between items-start mb-4">
                        <div>
