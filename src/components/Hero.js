...
export function Hero({ onNavigate }) {
    return (<div className="relative overflow-hidden bg-gradient-to-br from-pink-400 via-purple-500 to-indigo-600 min-h-screen">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 flex items-center min-h-screen">
        <div className="text-center w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center mb-16">
            <button onClick={() => onNavigate('borrow')} className="group bg-gradient-to-r from-pink-500 to-orange-500 text-white px-10 py-5 rounded-2xl text-xl font-bold hover:from-pink-600 hover:to-orange-600 transform hover:scale-110 hover:rotate-1 transition-all duration-300 shadow-2xl flex items-center justify-center space-x-3 border-2 border-white backdrop-blur-sm animate-pulse hover:animate-none">
              <span>Get Money Now! 💰</span>
              <ArrowRight className="h-6 w-6 group-hover:translate-x-2 transition-transform"/>
            </button>
            <button onClick={() => onNavigate('lend')} className="group bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-10 py-5 rounded-2xl text-xl font-bold hover:from-cyan-600 hover:to-blue-600 transform hover:scale-110 hover:-rotate-1 transition-all duration-300 shadow-2xl border-2 border-white backdrop-blur-sm animate-pulse hover:animate-none">
              Make Money! 📈
            </button>
          </div>
