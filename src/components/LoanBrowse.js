...
export function LoanBrowse({ onBack }) {
    ...
    const filteredLoans = sampleLoans.filter(loan => {
        if (filterType !== 'all' && loan.businessType !== filterType)
            return false;
        if (filterRisk !== 'all') {
            if (filterRisk === 'low' && loan.riskScore < 85)
                return false;
            if (filterRisk === 'medium' && (loan.riskScore < 70 || loan.riskScore > 85))
                return false;
            if (filterRisk === 'high' && loan.riskScore > 70)
                return false;
        }
        return true;
    });
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-cyan-400 via-teal-400 to-blue-500 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <button onClick={onBack} className="flex items-center space-x-2 text-white hover:text-yellow-300 font-bold text-lg bg-white bg-opacity-20 px-4 py-2 rounded-2xl backdrop-blur-sm transition-all duration-300 transform hover:scale-105 shadow-lg border-2 border-white border-opacity-30">
        ...
        <div className="bg-white bg-opacity-20 rounded-3xl shadow-2xl p-8 mb-8 backdrop-blur-sm border-2 border-white border-opacity-30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="w-full px-4 py-3 border-2 border-purple-300 rounded-2xl focus:ring-4 focus:ring-purple-200 focus:border-purple-400 bg-white font-bold text-lg text-gray-800 shadow-lg">
            ...
            <div>
              ...
              <select value={filterRisk} onChange={(e) => setFilterRisk(e.target.value)} className="w-full px-4 py-3 border-2 border-orange-300 rounded-2xl focus:ring-4 focus:ring-orange-200 focus:border-orange-400 bg-white font-bold text-lg text-gray-800 shadow-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredLoans.map((loan) => (<div key={loan.id} className="bg-white bg-opacity-20 rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:scale-105 backdrop-blur-sm border-2 border-white border-opacity-30">
              <div className="p-8">
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-white bg-opacity-30 rounded-full h-3 shadow-inner">
                    <div className="bg-gradient-to-r from-green-400 to-blue-500 h-3 rounded-full transition-all duration-300 shadow-lg" style={{ width: `${(loan.funded / loan.amount) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div className="flex space-x-3">
                  <button onClick={() => handleFund(loan.id)} className={`flex-1 py-3 rounded-lg font-semibold transition-all duration-200 ${loan.funded >= loan.amount
                ? 'bg-white bg-opacity-30 text-white cursor-not-allowed backdrop-blur-sm'
                : 'bg-gradient-to-r from-green-400 to-blue-500 text-white hover:from-green-500 hover:to-blue-600 transform hover:scale-105 border-2 border-white'}`} disabled={loan.funded >= loan.amount}>
        ...
        {filteredLoans.length === 0 && (<div className="text-center py-12 bg-white bg-opacity-20 rounded-3xl backdrop-blur-sm shadow-2xl border-2 border-white border-opacity-30">
